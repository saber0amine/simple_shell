...
/**
 * _trouveEnv - retrieves the value of an environment variable.
 * @name: string input
...
/**
 * _functionWich - locate the executable file associated with a given command.
 * @d: string input
 * Return: void
 */
...
{
        char *token, *path,
                *paths = malloc(_strlen(_trouveEnv("PATH") ? _trouveEnv("PATH") : "") + 1);
        size_t token_len;
        int find = -1;

        if (!_trouveEnv("PATH"))
                goto step_out;
        _strcpy(paths, _trouveEnv("PATH"));
        if (paths == NULL)
                goto step_out;
        token = strtok(paths, ":");
        while (token)
...
char **_new_environ(char *name, char *value)
{
        ...
        if (new_entry == NULL)
                return (NULL);
        new_environ = _trouveEnv(name) ? malloc((env_len + 1) * sizeof(char *))
...
/**
 * setenvFunction - Initialize a new environment variable, or modify an existing one
 * @d: to use the flag
 * @name: variable name
 * @value: variable value
 * Return: void
